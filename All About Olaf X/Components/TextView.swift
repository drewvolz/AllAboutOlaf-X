...
//  All About Olaf X
//
//  Created by Shaps on 7/25/20.
//  Modified by Drew Volz on 8/20/20
...
struct TextView_Previews: PreviewProvider {
    static var previews: some View {
        VStack(spacing: 5) {
            WrappedTextView("Placeholder",
                            text: .constant("The quick brown fox jumped over the lazy dog"),
                            isEditable: true)
                .font(.system(.body, design: .default))
                .placeholderFont(Font.system(.body, design: .default))
...
extension WrappedTextView {
    ...
    func autoDetectDataTypes(_ types: UIDataDetectorTypes) -> WrappedTextView {
        var view = self
        view.autoDetectionTypes = types
        return view
    }

    func foregroundColor(_ color: UIColor) -> WrappedTextView {
        var view = self
        view.foregroundColor = color
        return view
    }

    func autocapitalization(_ style: UITextAutocapitalizationType) -> WrappedTextView {
        var view = self
        view.autocapitalization = style
        return view
    }

    func multilineTextAlignment(_ alignment: TextAlignment) -> WrappedTextView {
        var view = self
    ...
    func font(_ font: UIFont) -> WrappedTextView {
        var view = self
        view.font = font
        return view
    }

    func placeholderFont(_ font: Font) -> WrappedTextView {
        var view = self
        view.placeholderFont = font
        return view
    }

    func fontWeight(_ weight: UIFont.Weight) -> WrappedTextView {
        font(font.weight(weight))
    }

    func clearOnInsertion(_ value: Bool) -> WrappedTextView {
        var view = self
        view.clearsOnInsertion = value
        return view
    }

    func disableAutocorrection(_ disable: Bool?) -> WrappedTextView {
        var view = self
    ...
    func isEditable(_ isEditable: Bool) -> WrappedTextView {
        var view = self
        view.isEditable = isEditable
        return view
    }

    func isSelectable(_ isSelectable: Bool) -> WrappedTextView {
        var view = self
        view.isSelectable = isSelectable
        return view
    }

    func enableScrolling(_ isScrollingEnabled: Bool) -> WrappedTextView {
        var view = self
        view.isScrollingEnabled = isScrollingEnabled
        return view
    }

    func returnKey(_ style: UIReturnKeyType?) -> WrappedTextView {
        var view = self
        view.returnKeyType = style
        return view
    }

    func automaticallyEnablesReturn(_ value: Bool?) -> WrappedTextView {
        var view = self
        view.enablesReturnKeyAutomatically = value
        return view
    }

    func truncationMode(_ mode: Text.TruncationMode) -> WrappedTextView {
        var view = self
        switch mode {
